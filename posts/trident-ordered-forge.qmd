...
So now we introduce a new flag `--ordered` to `trident forge`, which outputs samples not in the order of their original packages, but in the order that you, well, ordered them, if I may go back to my [restaurant analogy from an earlier blog post](trident_14.qmd).
