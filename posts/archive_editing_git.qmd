...
Note that this workflow assumes that you have installed and configured Git LFS on your system. Cloning the repo with the `GIT_LFS_SKIP_SMUDGE` environment variable prevents downloading the large LFS-tracked files despite Git LFS being enabled.
...
To validate a package completely it can be necessary to also access the genotype data. But because we cloned above with `GIT_LFS_SKIP_SMUDGE=1`, this data is not in our clone now. Fortunately we can selectively download it. Usually this works with `git lfs pull --include "PATH-TO-FILE"`.

But the Poseidon archive LFS setup complicates matters here, when accessing LFS data from a PR. This yet-to-be-merged data is usually not available from <https://lfs.poseidon-adna.org> yet. We thus have to either modify the `.lfsconfig` file by e.g. commenting out the `url` field, so that Git defaults back to GitHub for the LFS storage, or explicitly access this storage as follows:

```bash
git -c lfs.url=git@github.com:poseidon-framework/community-archive.git lfs pull \
    --include "PATH-TO-FILE"
```

`PATH-TO-FILE` can also include wildcards.

::: {.callout-tip}
## Edit note: 2026-01-16
The reference to <https://lfs.poseidon-adna.org> was added to this blog post after a major change in the Poseidon LFS infrastructure.
:::
